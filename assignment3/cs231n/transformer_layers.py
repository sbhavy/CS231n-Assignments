...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        pe[0, :, 0::2] = torch.sin(torch.arange(max_len)[:, None] * pows)
        pe[0, :, 1::2] = torch.cos(torch.arange(max_len)[:, None] * pows)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        Q = self.query(query).reshape(N, S, H, E // H)
        K = self.key(key).reshape(N, T, H, E // H)
        V = self.value(value).reshape(N, T, H, E // H)

        output = torch.matmul(Q.moveaxis(1,2), K.moveaxis(1,3)) / math.sqrt(E // H)

        if attn_mask is not None: output = output.masked_fill(attn_mask==0, float('-inf'))

        output = torch.matmul(self.attn_drop(F.softmax(output,-1)), V.moveaxis(1,2))
