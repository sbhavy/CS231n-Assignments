...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        encodings = self.positional_encoding(self.embedding(captions))
        projected = self.visual_projection(features).unsqueeze(1)
        attn_mask = torch.tril(torch.ones(T,T, device = encodings.device, dtype=encodings.dtype))

        scores = self.output(self.transformer(tgt=encodings,memory=projected, tgt_mask=attn_mask))
